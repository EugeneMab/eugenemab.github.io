...
const MalePerson: React.FC<MalePersonProps> = ({ person, descriptionScale, isFirstSelected, onClick, onUpdate, innerRef }) => {
  ...
  return (
    <div className="flex gap-4 items-start justify-end">
      <div className="flex flex-col text-right pt-2">
        <input
          className="font-bold text-lg leading-tight bg-transparent border-none text-right focus:ring-0 p-0"
          value={person.name}
          onChange={(e) => onUpdate(person.id, { name: e.target.value })}
        />
        <textarea
          className="text-sm text-gray-500 italic max-w-xs overflow-hidden bg-transparent border-none text-right focus:ring-0 p-0 resize-none"

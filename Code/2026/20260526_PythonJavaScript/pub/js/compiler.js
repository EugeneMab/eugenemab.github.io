...
export class Compiler {
    ...
    compileJS(program) {
        ...
        // Preload used built-ins that are not shadowed at top-level
        for (const b of BUILTINS) {
            if (usedBuiltins.has(b) && !this.topLevelVars.has(b)) {
    ...
    compileCall(node) {
        ...
        if (node.callee.type === "MemberAccess") {
            ...
            if (BUILTINS.has(member)) {
                // Use bare name if NOT shadowed in the current scope.
                // Mapped member calls use __runtime prefix if shadowed.
                const isShadowed = this.isShadowed(member);

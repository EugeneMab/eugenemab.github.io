...
export class Compiler {
    ...
    compileAssignment(node) {
        if (node.targets.length === 1 && node.targets[0].type === "Identifier") {
            return `${node.targets[0].name} = ${this.compileNode(node.value)}`;
        }
        ...
        const starName = node.targets[starIndex].name;
        ...
        if (node.callee.type === "MemberAccess") {
            ...
            if (BUILTINS.has(member)) {
                ...
                if (hasKeywords) {
                    ...
                    const kwArgs = kwArgsNodes.map((a) => `${a.name}: ${this.compileNode(a.value)}`);
                    const argsStr = `[${posArgs.join(", ")}], { ${kwArgs.join(", ")}, __is_kwargs: true }`;

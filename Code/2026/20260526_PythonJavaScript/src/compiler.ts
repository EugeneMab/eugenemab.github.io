...
export class Compiler {
  ...
  private compileFor(node: ForNode): string {
    if (node.iterable) {
      ...
      let js = `for await (const __tmp_item of ${iterable}) {\n`;
      this.indentLevel++;
      js += `${this.indent()}${node.iterator} = __tmp_item;\n`;
      for (const stmt of node.body) {
        const compiled = this.compileNode(stmt);
        if (compiled) js += `${this.indent()}${compiled};\n`;
      }
  ...
  private compileListComprehension(node: ListComprehensionNode): string {
    ...
    return `(await (async () => {
      const res = [];
      for await (const __tmp_item of ${iterable}) {
        const ${node.item} = __tmp_item;
  ...
  private compileWith(node: WithNode): string {
    ...
    const mgrVar = `__tmp_mgr_${this.indentLevel}`;
    let js = `{\n`;
    this.indentLevel++;
    js += `${this.indent()}const ${mgrVar} = ${expr};\n`;
    js += `${this.indent()}const __tmp_is_method = typeof ${mgrVar}.__enter__ === 'function';\n`;
    js += `${this.indent()}const __tmp_enter = __tmp_is_method ? ${mgrVar}.__enter__.bind(${mgrVar}) : runtime.__enter__;\n`;
    js += `${this.indent()}const __tmp_exit = typeof ${mgrVar}.__exit__ === 'function' ? ${mgrVar}.__exit__.bind(${mgrVar}) : runtime.__exit__;\n`;

    const enterCall = `await (__tmp_is_method ? __tmp_enter() : __tmp_enter(${mgrVar}))`;
    if (target) {
    ...
    this.indentLevel++;
    js += `${this.indent()}await (__tmp_is_method ? __tmp_exit(null, null, null) : __tmp_exit(${mgrVar}, null, null, null));\n`;
    this.indentLevel--;
    js += `${this.indent()}}\n`;
    this.indentLevel--;
    js += `${this.indent()}}`;

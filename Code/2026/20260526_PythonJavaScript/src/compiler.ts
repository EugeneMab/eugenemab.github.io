...
export class Compiler {
  ...
  private compileFor(node: ForNode): string {
    if (node.iterable) {
      ...
      // Use a temporary item to update the hoisted iterator (Python persistence)
      let js = `for await (const ${tmpItem} of ${iterable}) {\n`;
      this.indentLevel++;
  ...
  private compileCall(node: CallExpressionNode): string {
    ...
    if (node.callee.type === "MemberAccess") {
      ...
      return `(await (async () => {
        ...
        const ${tmpFallback} = runtime.${member};
  ...
  private compileSubscript(node: SubscriptNode): string {
    ...
    // Bind obj and idx to temporaries to avoid side-effect double-execution
    const tmpObj = this.nextTmp("obj");
    ...
    return `(await (async () => {
      const ${tmpObj} = ${obj};
      const ${tmpIdx} = ${idx};
      return ${tmpObj}[${tmpIdx} < 0 ? ${tmpObj}.length + ${tmpIdx} : ${tmpIdx}];
    })())`;
  }
  ...
  private compileListComprehension(node: ListComprehensionNode): string {
    ...
    return `(await (async () => {
      const res = [];
      for await (const ${tmpItem} of ${iterable}) {
  ...
  private compileWith(node: WithNode): string {
    ...
    js += `${this.indent()}const ${tmpEnter} = ${tmpIsMethod} ? ${tmpMgr}.__enter__.bind(${tmpMgr}) : runtime.__enter__;\n`;
    js += `${this.indent()}const ${tmpExit} = typeof ${tmpMgr}.__exit__ === 'function' ? ${tmpMgr}.__exit__.bind(${tmpMgr}) : runtime.__exit__;\n`;

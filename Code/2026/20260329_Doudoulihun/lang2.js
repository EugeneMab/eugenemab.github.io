...
const organizing = syls.map(s => convertSyllable(s, true)).join("");
const result = syls.map(s => convertSyllable(s, false)).join("");

const output = {
  input: input,
  generated: [breakdown, organizing, result]
};

fs.writeFileSync('compare2.json', JSON.stringify(output, null, 2));
console.log('Conversion for "investment" complete. Results saved to compare2.json');

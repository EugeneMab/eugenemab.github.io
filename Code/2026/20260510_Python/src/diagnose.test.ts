...
test("diagnose empty output issue", async ({ page }) => {
  ...
  const isLexHidden = await page
    .locator("#lex-content")
    .evaluate((el) => el.classList.contains("hidden"));
  console.log(
    `Lexicon Panel Visible: ${isLexVisible}, Has 'hidden' class: ${isLexHidden}`,
  );
  ...
  console.log(
    `Lexicon Text Length after manual click: ${lexContentAfterClick.length}`,
  );
});

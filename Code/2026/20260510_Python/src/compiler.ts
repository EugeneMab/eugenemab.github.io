...
export class Compiler {
  ...
  private tempLocal: number = 0;
  private comprehensionLocals: Map<ASTNode, number> = new Map();
  ...
  private emitFunctionBinary(node: FunctionDefNode): number[] {
    ...
    const scanNode = (n: ASTNode) => {
      ...
      if (n.type === "ListComprehension" || n.type === "DictComprehension") {
        ...
        this.localIndex += 5;
        for (let i = 0; i < 5; i++) localTypes.push(0x7f);
      }
      switch (n.type) {
        case "Assignment":
          scanNode(n.value);
  ...
  private emitExpressionBinary(node: ASTNode): number[] {
    switch (node.type) {
      case "Literal":
        ...
        return [];
      case "List":
        const elementsBytes: number[] = [];
        node.elements.forEach((el) =>
          elementsBytes.push(...this.emitExpressionBinary(el)),
        );
        ...
        for (let i = length - 1; i >= 0; i--)
          elementsBytes.push(
            0x21,
            ...this.encodeUnsignedLEB128(this.tempLocal + 1),
            0x20,
            ...this.encodeUnsignedLEB128(this.tempLocal),
            0x41,
            ...this.encodeSignedLEB128((i + 1) * 4),

...
export function initUI() {
  ...
  const runCode = () => {
    ...
    worker.onmessage = (e) => {
      ...
      switch (type) {
        case 'lex': outputs.lex.textContent = JSON.stringify(payload, null, 2); break;
        case 'ast': outputs.ast.textContent = JSON.stringify(payload, null, 2); break;
        case 'wat': outputs.wat.textContent = payload; break;
        case 'wasm': outputs.wasm.textContent = Array.from(payload as Uint8Array).map(b => b.toString(16).padStart(2, '0')).join(' '); break;
        case 'log': outputs.exec.textContent += payload + '\n'; break;
        case 'result': 

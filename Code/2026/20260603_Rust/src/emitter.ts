import { Program, Statement, Expression, LetStatement, BinaryExpression, Literal, Identifier, FunctionDeclaration, MacroInvocation, BlockStatement } from './parser.js';
...
export class Emitter {
  ...
  private emitFunction(fn: FunctionDeclaration) {
  ...
  private emitLocals(block: BlockStatement) {
  ...
  private emitBlock(block: BlockStatement) {
    for (const stmt of block.body) {
      if (stmt.type === 'LetStatement') {
  ...
  private emitBinaryOp(op: string) {
    switch (op) {
      case '+': this.emitLine('i32.add'); break;
      case '-': this.emitLine('i32.sub'); break;
      case '*': this.emitLine('i32.mul'); break;
      case '/': this.emitLine('i32.div_s'); break;
      case '%': this.emitLine('i32.rem_s'); break;
      case '&': this.emitLine('i32.and'); break;
      case '|': this.emitLine('i32.or'); break;
      case '^': this.emitLine('i32.xor'); break;
      case '<<': this.emitLine('i32.shl'); break;
      case '>>': this.emitLine('i32.shr_s'); break;
    }
  }

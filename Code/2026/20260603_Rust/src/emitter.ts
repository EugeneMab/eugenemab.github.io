import { Program, Statement, Expression, FunctionDeclaration, BlockStatement } from './parser.js';
...
export class Emitter {
    ...
    switch (expr.type) {
      ...
      case 'BinaryExpression':
        ...
        switch (expr.operator) {
          case '+': this.emitWATLine('i32.add'); break;
          case '-': this.emitWATLine('i32.sub'); break;
          case '*': this.emitWATLine('i32.mul'); break;
          case '/': this.emitWATLine('i32.div_s'); break;
          case '%': this.emitWATLine('i32.rem_s'); break;
          case '&': this.emitWATLine('i32.and'); break;
          case '|': this.emitWATLine('i32.or'); break;
          case '^': this.emitWATLine('i32.xor'); break;
          case '<<': this.emitWATLine('i32.shl'); break;
          case '>>': this.emitWATLine('i32.shr_s'); break;
        }
        break;
  ...
  emitWASM(): Uint8Array {
    ...
    const userFunctions = this.program.body.filter(s => s.type === 'FunctionDeclaration') as FunctionDeclaration[];
    ...
    const codeSection = this.encodeSection(SECTION_CODE, this.encodeVector(
      userFunctions.map(fn => this.emitFunctionBinary(fn))
    ));
  ...
  private emitFunctionBinary(fn: FunctionDeclaration): number[] {
    ...
    const localNames = new Set<string>();
    const scan = (s: Statement) => {
      if (s.type === 'LetStatement') if (!this.locals.has(s.name)) localNames.add(s.name);
      if (s.type === 'BlockStatement') s.body.forEach(scan);
    };
    fn.body.body.forEach(scan);
  ...
  private emitExpressionBinary(expr: Expression, body: number[]) {
    switch (expr.type) {
      ...
      case 'BinaryExpression':
        ...
        switch (expr.operator) {
          case '+': body.push(OP_I32_ADD); break;
          case '-': body.push(OP_I32_SUB); break;
          case '*': body.push(OP_I32_MUL); break;
          case '/': body.push(OP_I32_DIV_S); break;
          case '%': body.push(OP_I32_REM_S); break;
          case '&': body.push(OP_I32_AND); break;
          case '|': body.push(OP_I32_OR); break;
          case '^': body.push(OP_I32_XOR); break;
          case '<<': body.push(OP_I32_SHL); break;
          case '>>': body.push(OP_I32_SHR_S); break;
        }
        break;
      case 'MacroInvocation':
        if (expr.name === 'print') {

...
export class Emitter {
    ...
    enumDefinitions = new Map();
    moduleUseMappings = new Map();
    itemIsPublic = new Map();
    ...
    prepareFunctionMetadata() {
        ...
        const userFunctions = [];
        const collect = (statements, prefix) => {
            ...
            statements.forEach((s) => {
                ...
                else if (s.type === "RegularStructDeclaration" ||
        ...
        userFunctions.forEach((fn, i) => {
            ...
            const resolvedReturnType = fn.returnType
                ...
                : undefined;
            this.functionReturnTypes.set(fn.name, this.normalizeType(resolvedReturnType));
        });
    ...
    emitWAT() {
        ...
        this.emitWATLine('(import "env" "get_item" (func $get_item (param i32 i32) (result i32)))');
        this.emitWATLine('(import "env" "get_item_i32" (func $get_item_i32 (param i32 i32) (result i32)))');
        this.emitWATLine('(import "env" "set_item" (func $set_item (param i32 i32 i32) (result i32)))');
        this.emitWATLine('(import "env" "set_item_i32" (func $set_item_i32 (param i32 i32 i32) (result i32)))');
        ...
        for (const stmt of block.body) {
            if (stmt.type === "LetStatement") {
                ...
                if (path.startsWith("crate::"))
                    path = path.substring(7);
                const alias = path.split("::").pop();
                this.scopeStack[this.scopeStack.length - 1].useMappings.set(alias, path);
            }
            ...
            if (!foundMapping) {
                if (name.startsWith("super::")) {
                    ...
                    while (parts.length > 0) {
                        ...
                        if (this.functionIndices.has(fullName) ||
    ...
    checkVisibility(path, context) {
        if (this.functionIndices.has(path) ||
            ...
            for (let i = 0; i < parts.length; i++) {
                ...
                if (isPublic === false) {
                    let parent = currentPath.includes("::")
                        ...
                        : "";
                    if (this.structDefinitions.has(parent) ||
                    ...
                    let ctxModule = ctx.includes("::")
                        ...
                        : "";
                    if (this.structDefinitions.has(ctxModule) ||
                        this.enumDefinitions.has(ctxModule)) {
                        ctxModule = ctxModule.includes("::")
                            ? ctxModule.split("::").slice(0, -1).join("::")
                            : "";
                    }
                    const isInsideParent = parent === "" || ctxModule.startsWith(parent + "::");
        ...
        switch (expr.type) {
            ...
            case "MatchExpression": {
                ...
                for (let i = 0; i < expr.arms.length; i++) {
                    ...
                    if (arm.pattern &&
                        ...
                        if (enumDef) {
                            variantIndex = enumDef.variants.findIndex((v) => v.name === variantName);
                        }
                    }
            ...
            case "IndexExpression": {
                ...
                if (expr.index.type === "RangeExpression") {
                    ...
                    const objectType = this.inferExpressionType(expr.object);
                    const isI32Array = this.isArrayLikeType(objectType) &&
                        !this.isByteLikeType(objectType) &&
                        !this.isStringLikeType(objectType);
                        ...
                        while ((match = regex.exec(formatStr)) !== null) {
                            ...
                            if (varName) {
                                ...
                                if (argIndex < expr.args.length) {
                                    ...
                                    this.throwError("Not enough arguments for format string", expr);
                                }
                            }
    ...
    emitWASM() {
        ...
        const funcSection = this.encodeSection(SECTION_FUNCTION, this.encodeVector([
        ...
        const dataSection = dataEntries.length > 0
            ...
            if (stmt.type === "LetStatement") {
                ...
                if (stmt.elseBranch) {
                    ...
                    if (stmt.elseBranch.type === "BlockStatement") {
                        ...
                        this.emitBlockBinary({ type: "BlockStatement", body: [stmt.elseBranch] }, body);
                    }
                }
                else {
        ...
        if (expr.type === "BinaryExpression") {
            ...
            if (arithmeticOps.includes(expr.operator)) {
                ...
                if (leftType !== rightType) {
                    this.throwError(`Type error: cannot apply '${expr.operator}' to types ${leftType} and ${rightType}`, expr);
                }
            }
        }
        switch (expr.type) {
            ...
            case "MatchExpression": {
                ...
                body.push(OP_LOCAL_GET, 0xfe, ptrLocal);
                body.push(...OP_I32_LOAD);
                body.push(OP_LOCAL_SET, 0xfe, tagLocal);
                let opens = 0;
                for (let i = 0; i < expr.arms.length; i++) {
                    const arm = expr.arms[i];
                    const isLast = i === expr.arms.length - 1;
                    ...
                    body.push(OP_LOCAL_GET, 0xfe, tagLocal);
            ...
            case "IndexExpression": {
                ...
                if (expr.index.type === "RangeExpression") {
                    ...
                    body.push(OP_CALL, ...this.encodeUnsignedLEB128(this.functionIndices.get(isI32Array ? "get_item_i32" : "get_item")));
                }
                break;
            }
            case "Identifier": {
    ...
    emitStringPrintBinary(str, body) {
        if (!this.stringConstants.has(str)) {
            this.stringConstants.set(str, this.stringOffset);
            this.stringOffset += 4 + new TextEncoder().encode(str).length;
        }
